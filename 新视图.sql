CREATE OR REPLACE VIEW V_PM_ALP_RELEASE_PROJECT AS
SELECT BB.BUNCH_NO,
       BB.BUNCH_NAME,
  ...
  FROM (SELECT PB.BUNCH_NO,
          ...
          FROM SGIRS.PM_BUNCH PB
               ...
               (SELECT JJ.PRJ_CODE,
                       ...
                       AA.TOTAL_FUND AS Y_TOTAL_FUND,
                       AA.CAPITAL_FUND AS Y_CAPITAL_FUND,
                       AA.COST_FUND AS Y_COST_FUND,
                       KK.TOTAL_FUND AS Z_TOTAL_FUND,
                       KK.CAPITAL_FUND AS Z_CAPITAL_FUND,
                       KK.COST_FUND AS Z_COST_FUND,
                       JJ.PF_SORT_CODE,
                       JJ.PT_SORT_CODE,
                       JJ.PRJ_NATURE_CODE,
                       '0' AS ISBUNCH
                  FROM (SELECT PAD.PRJ_CODE,
                          ...
                          FROM SGIRS.PM_ALP_DHT PAD
                               ...
                               LEFT JOIN SGIRS.PM_ALP_FHT PAF ON PAD.VER_ID = PAF.VER_ID AND PAD.DETAIL_ID = PAF.DETAIL_ID
                               INNER JOIN
                         ...
                         WHERE PAD.EDIT_TYPE_CODE = '02'
                           AND PAF.FUND_TYPE = '03'
                         GROUP BY PAD.PRJ_CODE,
                                  PBI.PRJ_NAME,
                                  PBI.BLG_ORG_NO,
                                  PBI.APP_ORG_NO,
                                  PBI.APP_DEPT_NO,
                       ...
                       (SELECT PAD.PRJ_CODE,
                         ...
                         GROUP BY PAD.PRJ_CODE,
                                  ...
                                  PAD.PF_SORT_CODE,
                                  PAD.PT_SORT_CODE,
                                  PBI.PRJ_NATURE_CODE) KK ON JJ.RPT_YEAR = KK.RPT_YEAR AND JJ.BLG_ORG_NO = KK.BLG_ORG_NO AND JJ.PRJ_CODE = KK.PRJ_CODE
                         LEFT JOIN
                          (SELECT PAD.PRJ_CODE,
                               PBI.PRJ_NAME,
                               PBI.BLG_ORG_NO,
                         ...
                         GROUP BY PAD.PRJ_CODE,
                                  ...
                                  PAD.PF_SORT_CODE,
                                  PAD.PT_SORT_CODE,
                                  PBI.PRJ_NATURE_CODE) AA ON JJ.RPT_YEAR = AA.RPT_YEAR AND JJ.BLG_ORG_NO = AA.BLG_ORG_NO AND JJ.PRJ_CODE = AA.PRJ_CODE
                                  ) SS ,
               (SELECT T2.RPT_YEAR,
                       MAX(TO_NUMBER(VER_NO)) AS VER_NO,
                       T2.ORG_NO
                  FROM SGIRS.PM_ALP_RELEASE T2
                 GROUP BY T2.RPT_YEAR, T2.ORG_NO) MM
         WHERE PB.BUNCH_YEAR = SS.RPT_YEAR AND PBD.PRJ_CODE = SS.PRJ_CODE
         AND PB.ORG_NO = MM.ORG_NO AND PB.VER_ID = MM.VER_NO AND PB.BUNCH_YEAR = MM.RPT_YEAR
...
SELECT PP.PRJ_CODE,
  ...
  FROM (SELECT PAD.PRJ_CODE,
         ...
         WHERE PAD.EDIT_TYPE_CODE = '02'
           ...
           AND  NOT EXISTS
               (SELECT NULL
                 ...
                 WHERE PB.BUNCH_ID = PBD.BUNCH_ID AND TO_NUMBER(PB.VER_ID) = GG.VER_NO AND PB.ORG_NO = GG.ORG_NO AND PAD.PRJ_CODE= PBD.PRJ_CODE)
         GROUP BY PAD.PRJ_CODE,
                  PBI.PRJ_NAME,
                  PBI.BLG_ORG_NO,
                  PBI.APP_ORG_NO,
       ...
       (SELECT PAD.PRJ_CODE,
               ...
               PBI.PLAN_END_DATE,
               PAR.RPT_YEAR,
               PAF.TOTAL_FUND AS Z_TOTAL_FUND,
               PAF.CAPITAL_FUND AS Z_CAPITAL_FUND,
               PAF.COST_FUND AS Z_COST_FUND,
               PAD.PF_SORT_CODE,
               PAD.PT_SORT_CODE,
               PBI.PRJ_NATURE_CODE,
               '0' AS ISBUNCH
         ...
         GROUP BY PAD.PRJ_CODE,
                  ...
                  PAD.PF_SORT_CODE,
                  PAD.PT_SORT_CODE,
                  PBI.PRJ_NATURE_CODE) LL ON PP.RPT_YEAR = LL.RPT_YEAR AND PP.BLG_ORG_NO = LL.BLG_ORG_NO AND PP.PRJ_CODE = LL.PRJ_CODE
         LEFT JOIN
          (SELECT PAD.PRJ_CODE,
               PBI.PRJ_NAME,
               PBI.BLG_ORG_NO,
               ...
               PBI.PLAN_END_DATE,
               PAR.RPT_YEAR,
               PAF.TOTAL_FUND AS Y_TOTAL_FUND,
               PAF.CAPITAL_FUND AS Y_CAPITAL_FUND,
               PAF.COST_FUND AS Y_COST_FUND,
               PAD.PF_SORT_CODE,
               PAD.PT_SORT_CODE,
               PBI.PRJ_NATURE_CODE,
               '0' AS ISBUNCH
         ...
         GROUP BY PAD.PRJ_CODE,
                  ...
                  PAD.PF_SORT_CODE,
                  PAD.PT_SORT_CODE,
                  PBI.PRJ_NATURE_CODE) VV ON PP.RPT_YEAR = VV.RPT_YEAR AND PP.BLG_ORG_NO = VV.BLG_ORG_NO AND PP.PRJ_CODE = VV.PRJ_CODE;
